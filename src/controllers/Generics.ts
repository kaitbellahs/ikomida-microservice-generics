...
export default class Generics {
  ...
  async getTerm(type?: Types.Types.TTerm | null) {
    try {
      ...
      const term = Types.Classes.CTerm.init(termModel?.name ?? '', termModel?.text ?? '', termModel?.type ?? Types.Types.TTerm.PRIVACY_POLICY, undefined, termModel?.createdAt, termModel?.id);
      return new Utils.Return(termModel !== null, term);
    } catch (exception: any) {
      new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_USERS_SERVICE_ADDRESS_GET_ADDRESS_EXCEPTION, exception).log(this.logger);
    }
  ...
  async getLastTerm(type?: Types.Types.TTerm | null) {
    try {
      ...
      const term = Types.Classes.CTerm.init(termModel?.name ?? '', '', termModel?.type ?? Types.Types.TTerm.PRIVACY_POLICY, undefined, termModel?.createdAt, termModel?.id);
      return new Utils.Return(termModel !== null, term);
    } catch (exception: any) {
      new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_USERS_SERVICE_ADDRESS_GET_ADDRESS_EXCEPTION, exception).log(this.logger);
    }
  ...
  async getAddressByCep(postalCode: string) {
    try {
      ...
      if ((postalCodeJustNumbersArray?.length ?? 0) !== 8) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_USERS_SERVICE_ADDRESS_GET_ADDRESS_INVALID_POSTAL_CODE);
      ...
      const address = Types.Classes.CAddress.init(postalCodeJustNumbers, response.data?.logradouro ?? '', response.data?.bairro ?? '', response.data?.localidade ?? '', response.data?.uf ?? '', undefined, response.data?.complemento ?? '');
